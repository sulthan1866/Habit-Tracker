...
const Email = ({ isSendingEmail, setSendingEmail }: Props) => {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      const result = await axios.post(
        `${import.meta.env.VITE_BASE_API_URL_V1}/send-register`,
        { email }
      );

      if (result.status == 202) {
        setMessStyle("info");
        setMessage("Registeration link sent successfully");
      } else {
        setMessStyle("danger");
      ...
      setMessStyle("danger");
      setMessage("User with this E-Mail already exixts");
    } finally {
      ...
      >
        <div className="row justify-content-center">
          <div className="card p-4 shadow">
            ...
            <h3 className="text-center mb-4">Get regiatration link</h3>
            {Message && (
              <div
